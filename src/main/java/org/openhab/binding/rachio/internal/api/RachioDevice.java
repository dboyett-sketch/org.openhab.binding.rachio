...
public RachioDevice(RachioCloudDevice device, ThingUID bridgeUID, ThingUID deviceUID) {
    try {
        setUID(bridgeUID, deviceUID); // ✅ MUST be first to initialize dev_uid

        RachioApi.copyMatchingFields(device, this);
        logger.trace("RachioDevice: Adding device '{}' (id='{}', model='{}', on={}, status={}, deleted={})",
        ...
        if (!device.deleted) {
            ...
            for (int i = 0; i < device.zones.size(); i++) {
                ...
                if (true /* zone.enabled */) {
                    RachioZone zoneObj = new RachioZone(zone, getThingID()); // ✅ safe now
                    zoneObj.setUID(deviceUID, new ThingUID("rachio:zone:" + deviceUID.getId() + ":" + zone.id));
                    zoneObj.setBridgeUID(bridgeUID);
                    zoneList.put(zone.id, zoneObj);
                } else {
                    logger.trace("RachioDevice: Zone '{}.{}[{}]' is disabled, skip.", device.name, zone.name, zone.zoneNumber);
                }
            }
        }
    } catch (Exception e) {
    ...
    return dev_uid != null ? dev_uid.getId() : "unknown";
    }
